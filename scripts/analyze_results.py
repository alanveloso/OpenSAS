...
def filter_request_types(all_data: pd.DataFrame) -> pd.DataFrame:
    """Filtra os dados removendo tipos de requisição que não devem ser analisados."""
    # Lista de tipos de requisição que devem ser excluídos da análise
    excluded_types = [
        'Generate Valid SAS Address',
        'Generatevalidsasaddress',
        'Generate Valid Sas Address',
        'generatevalidsasaddress'
    ]
    
    # Filtra removendo os tipos excluídos
    filtered_data = all_data[~all_data['request_type'].isin(excluded_types)].copy()
    
    if len(filtered_data) < len(all_data):
        removed_count = len(all_data) - len(filtered_data)
        print(f"[INFO] Removidos {removed_count} registros de tipos de requisição excluídos")
...
def load_data(jtl_files: List[Path]) -> pd.DataFrame:
    ...
    if dfs:
        ...
        # Filtra tipos de requisição que não devem ser analisados
        all_data = filter_request_types(all_data)
        
        # Mostra os tipos de requisição após filtragem
        filtered_types = sorted(all_data['request_type'].unique())
        print(f"[INFO] Tipos de requisição após filtragem: {filtered_types}")
